...
export function ItemContextPills({
  ...
  const kindFilteredCategories = availableCategories.filter((category) => {
    if (!itemKind) {
      return true;
    }
    return !category.applies_to_kind || category.applies_to_kind === itemKind;
  });
  const managedCategoryOptions = kindFilteredCategories.map((category) => ({
    value: category.key,
